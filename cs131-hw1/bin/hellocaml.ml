...
let compose_pair (p : ('b -> 'c) * ('a -> 'b)) : 'a -> 'c =
  match p with p1, p2 -> fun (x : 'a) -> p1 (p2 x) 
...
let rec list_to_mylist (l : 'a list) : 'a mylist =
  match l with [] -> Nil | h  :: rest -> Cons (h ,list_to_mylist(rest))
...
let rec append (l1 : 'a list) (l2 : 'a list) : 'a list =
  match l1 with [] -> l2 | h  :: rest -> h :: append rest l2
...
let rec rev (l : 'a list) : 'a list =
  match l with [] -> [] | h  :: rest -> append (rev(rest)) (h :: [])
...
let rec insert (x : 'a) (l : 'a list) : 'a list =
  match l with
  |[] -> x :: []
  |h :: rest -> if h < x then h :: insert x rest else if h = x then l else x :: l
...
let rec lookup (x : string) (c : ctxt) : int64 =
  ...
  |[] -> raise Not_found
  |h :: rest -> match h with
    |(var, v) -> if var = x then v else lookup x rest
...
let rec optimize (e : exp) : exp =
  ...
  |Var e1 -> e
  |Const e1 -> e
  |Neg e1 -> (match optimize e1 with Const n -> Const (Int64.neg n) | e1' -> e)
  |Add (e1, e2) ->(
  ...
  |Mult (e1, e2) ->(
    ...
    |(Const 1L, e1') -> e1'
    |(e1', Const 1L) -> e1'
    |(Const 0L, e1') -> Const 0L
    |(e1', Const 0L) -> Const 0L
    |(e1', e2') -> Mult (e1', e2')
  )
